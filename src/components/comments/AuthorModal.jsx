...
export default function AuthorModal({ userId, isOpen, onClose }) {
  ...
  const { mutedIds, blockedIds, mute, unmute, block, unblock } = useUserFilters();
  ...
  const handleMuteToggle = async () => {
    ...
    setFilterError(null);
    try {
      await (isMuted ? unmute(userId) : mute(userId));
    } catch (err) {
  ...
  const handleBlockToggle = async () => {
    if (!isBlocked && !window.confirm(
      `Block ${profile?.name || "this member"}? Their comments disappear for you, and neither of you can DM the other.`
    )) return;
    setFilterBusy(true);
    setFilterError(null);
    try {
                ...
                <>
                  ...
                  <div className="flex gap-2 mt-2">
                    <button
                      ...
                      disabled={filterBusy}
                      className="flex-1 py-2 rounded-lg text-sm font-medium bg-slate-100 dark:bg-slate-700/60 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors disabled:opacity-50"
                    >
                      {isMuted ? "Unmute" : "Mute"}
                    </button>
                    <button
                  ...
                  {filterError && (
                    <p className="text-xs text-red-500 dark:text-red-400 mt-1 text-center">{filterError}</p>
                  )}
                </>
              )}
